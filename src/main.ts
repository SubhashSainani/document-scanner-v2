...
// Adaptive: samples background brightness and switches strategy
// ═══════════════════════════════════════

/**
 * Sample the average brightness of the four corner regions of a grayscale Mat.
 * Used to decide whether we're on a light or dark background.
 */
function sampleBackgroundBrightness(gray: any, w: number, h: number): number {
  const s = Math.max(8, Math.round(Math.min(w, h) * 0.12));
  ...
  for (const [ox, oy] of corners) {
    for (let dy = 0; dy < s; dy += 3) {
      for (let dx = 0; dx < s; dx += 3) {
        ...
        if (px < w && py < h) { sum += gray.ucharAt(py, px); count++; }
      }
    }
  }
...
/**
 * Run one detection pass with a given Canny threshold pair.
 * Optionally applies CLAHE contrast enhancement before edge detection.
 * Returns the best 4-corner polygon and its area, or null.
 */
function tryDetectStrategy(
  gray: any, kernel: any, imgArea: number,
  cannyLow: number, cannyHigh: number, useClahe: boolean,
): { corners: [number, number][]; area: number } | null {
  let enhanced: any, blurred: any, edges: any, dilated: any, contours: any, hierarchy: any;
  try {
    enhanced  = new cv.Mat();
    blurred   = new cv.Mat();
    edges     = new cv.Mat();
    ...
    if (bestApprox && bestArea > imgArea * 0.05) {
      ...
      bestApprox.delete();
      return { corners: orderCorners(pts), area: bestArea };
    }
    bestApprox?.delete();
    return null;
  } catch {
    ...
    enhanced?.delete(); blurred?.delete(); edges?.delete();
...
function detectDocumentCorners(canvas: HTMLCanvasElement): [number, number][] | null {
  let src: any, gray: any, kernel: any;
  try {
    ...
    // Measure how bright the corners of the frame are
    const bgBrightness = sampleBackgroundBrightness(gray, w, h);
    const isLight = bgBrightness > 145; // light desk / white paper on white background

    // Strategy order:
    //  Light background → start with CLAHE + low thresholds (catches subtle edges / shadows)
    //  Dark background  → start with standard thresholds (high contrast, works immediately)
    const strategies: Array<[number, number, boolean]> = isLight
      ? [
          [15, 50,  true ],  // CLAHE + very sensitive — best for white-on-white
          [25, 80,  true ],  // CLAHE + moderate
          [50, 150, false],  // standard fallback
        ]
      : [
          [50, 150, false],  // standard — best for dark backgrounds
          [30, 100, false],  // lower thresholds fallback
          [15, 50,  true ],  // CLAHE last resort
        ];

    let bestCorners: [number, number][] | null = null;
    let bestArea = 0;

    for (const [lo, hi, clahe] of strategies) {
      const res = tryDetectStrategy(gray, kernel, imgArea, lo, hi, clahe);
      if (res && res.area > bestArea) {
        bestArea = res.area;
        bestCorners = res.corners;
        // Good enough result — stop early to save time
        if (bestArea > imgArea * 0.10) break;
      }
    }
