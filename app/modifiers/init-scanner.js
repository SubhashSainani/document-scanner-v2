...
export default modifier(function initScanner(element) {
    ...
    const cornerCanvas = el('corner-canvas');
    const resultCanvas = el('result-canvas');
    const magnifierDiv = el('magnifier');
    const loadingOverlay = el('loading-overlay');
    const loadingText = el('loading-text');
    const detectBadge = el('detect-badge');
    const scanGuide = el('scan-guide');
    const exportModal = el('export-modal');
    const camHint = el('cam-hint');
    const startScanBtn = el('start-scan-btn');
    const closeCameraBtn = el('close-camera-btn');
    const captureBtn = el('capture-btn');
    const retakeBtn = el('retake-btn');
    const confirmBtn = el('confirm-btn');
    const autoFixBtn = el('auto-fix-btn');
    const scanAgainBtn = el('scan-again-btn');
    const saveImageBtn = el('save-image-btn');
    const exportPdfBtn = el('export-pdf-btn');
    const shareBtn = el('share-btn');
    const clearRecentsBtn = el('clear-recents-btn');
    const modalCancel = el('modal-cancel');
    const modalExport = el('modal-export');
    const recentsGrid = el('recents-grid');
    function el(id) {
  ...
  `;
    ...
    function runDetection() {
        ...
        if (found) {
            liveCorners = found.map(([x, y]) => [x / scale, y / scale]);
            drawOverlay(liveCorners);
            detectBadge.classList.add('visible');
            scanGuide.classList.add('detected');
            camHint.textContent = 'Document detected â€” tap to capture';
        }
        else {
            liveCorners = null;
            clearOverlay();
    ...
    function captureFrame() {
        ...
        tmp.width = capturedWidth;
        tmp.height = capturedHeight;
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // Corner Canvas â€” manual adjustment
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    // Magnifier
    function drawMagnifier(canvasX, canvasY, clientX, clientY) {
        const SIZE = 80;
        const ZOOM = 2.5;
        ...
        ctx.drawImage(previewCanvas, srcX, srcY, srcW, srcH, 0, 0, SIZE, SIZE);
        // Crosshair
    ...
    function autoFix() {
        ...
        if (detected) {
            ...
            hint.textContent = 'No document edges found â€” adjust manually';
    ...
    function processDocument() {
        ...
        setTimeout(() => {
            ...
            try {
                ...
                srcCanvas.width = capturedWidth;
                srcCanvas.height = capturedHeight;
                srcCanvas.getContext('2d').putImageData(capturedImageData, 0, 0);
                src = cv.imread(srcCanvas);
                // Inset corners â€” top gets a larger nudge
    ...
    function saveRecentScan() {
        try {
            ...
            if (ratio >= 1) {
                ...
                thumb.height = Math.round(THUMB_SIZE / ratio);
            }
            else {
                thumb.height = THUMB_SIZE;
                thumb.width = Math.round(THUMB_SIZE * ratio);
            }
            ...
            if (stored.length > RECENT_MAX)
                stored.length = RECENT_MAX;
    ...
    function renderRecents() {
        ...
        <div class="no-recents">
          ...
          <span>No recent scans yet</span>
            ...
            recentsGrid.innerHTML = stored.map((url, i) => `<div class="recent-thumb" data-idx="${i}"><img src="${url}" alt="Scan ${i + 1}" loading="lazy"></div>`).join('');
        }
        catch { }
    }
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
